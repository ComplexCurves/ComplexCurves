...
void main(void) {
    if (barycentric == vec3 (1.0, 0.0, 0.0)) {
        if (computedRoots >= sheets) {
            fColor = texture (oldSamplers[0], texCoord[0]);
        } else {
            int i = computedRoots / 2 + 1;
            for (int j = 1; j < 1 + N/2; j++)
                if (j == i)
                    fColor = texture (oldSamplers[j], texCoord[0]);
        }
    } else if (barycentric == vec3 (0.0, 1.0, 0.0)) {
        if (computedRoots >= sheets) {
            fColor = texture (oldSamplers[0], texCoord[1]);
        } else {
            int i = computedRoots / 2 + 1;
            for (int j = 1; j < 1 + N/2; j++)
                if (j == i)
                    fColor = texture (oldSamplers[j], texCoord[1]);
        }
    } else if (barycentric == vec3 (0.0, 0.0, 1.0)) {
        if (computedRoots >= sheets) {
            fColor = texture (oldSamplers[0], texCoord[2]);
        } else {
            int i = computedRoots / 2 + 1;
            for (int j = 1; j < 1 + N/2; j++)
                if (j == i)
                    fColor = texture (oldSamplers[j], texCoord[2]);
        }
    } else {
        vec2 values[N];
        vec2 position = barycentric.x * texture (oldSamplers[0], texCoord[0]).xy
